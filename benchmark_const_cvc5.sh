...
for s in "${test_size[@]}"; do
  ...
  timeout 10s ./build/mlir-tv ${src_file} ${tgt_file} --associative --cvc-only 2>/dev/null | tail -n2 -f
done
...
for s in "${test_size[@]}"; do
  ...
  timeout 10s ./build/mlir-tv ${src_file} ${tgt_file} --associative --cvc-only --hashfn 2>/dev/null | tail -n2 -f
done
