...
static tuple<State, State, Expr> encodeFinalStates(
  ...
  if (aop::getAddAssociativity() && vinput.useHash)
...
static void checkIsSrcAlwaysUB(
  ...
  aop::setAbstractionLevel(aop::AbsLevelDot::SUM_MUL, vinput.associativeAdd, vinput.useHash);
...
static Results validate(ValidationInput vinput) {
  ...
  if (assocAdd || useSumMulForDot) {
    aop::setAbstractionLevel(aop::AbsLevelDot::SUM_MUL, assocAdd, vinput.useHash);
